...
class Telegram extends Core {
  ...
  async getJob (config) {
    let jobs = await this.Agrobase.getJobs({
      jobType: Contants.AMBIENTAL_ENGINEER_AGROBASE,
      page: 0
    });
    return jobs;
  }
  ...
  run () {
    ...
    bot.command('/jobs', async (ctx) => {
      ...
      if (text.split(' ').length <= 1) {
        ...
        const jobs = await Request(`https://jobs.github.com/positions.json?description=${desc}&location=${city}`);
    ...
    bot.command('/agrobase', async (ctx) => {
      ...
      const availableJobs = [
        { name: '1) Engenheiro Ambiental', type: Contants.AMBIENTAL_ENGINEER_AGROBASE },
        { name: '2) Engenheiro Florestal', type: Contants.FLORESTAL_ENGINEER_AGROBASE },
        { name: '3) Nutricionista', type: Contants.NUTRICIONIST_AGROBASE }
      ];
      ...
      if (text.split(' ').length <= 1) {
        ...
        let jobs = await this.Agrobase.getJobs({
          jobType: availableJobs[opt].type,
          page
        });
        ...
        await this.reply(ctx, 'Digite */mais* para receber mais empregos');
      }
    });
